...
G2_DTS_FILES = $(wildcard $(TOP)/$(TARGET_KERNEL_SOURCE)/arch/arm/boot/dts/msm8974/msm8974-sec-js01ltejpn*.dts)
...
$(INSTALLED_BOOTIMAGE_TARGET): $(MKBOOTIMG) $(INTERNAL_BOOTIMAGE_FILES) $(INSTALLED_DTIMAGE_TARGET)
	...
	@echo -e ${CL_CYN}"Lies about SEANDROIDENFORCE to Samsung Bootloader"${CL_RST}
	$(hide) echo -n "SEANDROIDENFORCE" >> $(PRODUCT_OUT)/boot.img
	@echo -e ${CL_CYN}"Made boot image: $@"${CL_RST}
	$(hide) tar --create --format=ustar --directory $(PRODUCT_OUT) boot.img > $(PRODUCT_OUT)/boot.tar
	@echo -e ${CL_CYN}"Made Odin boot image: $(PRODUCT_OUT)/boot.tar"${CL_RST}
...
$(INSTALLED_RECOVERYIMAGE_TARGET): $(MKBOOTIMG) $(INSTALLED_DTIMAGE_TARGET) \
	...
	$(hide) tar --create --format=ustar --directory $(PRODUCT_OUT) recovery.img > $(PRODUCT_OUT)/recovery.tar
	@echo -e ${CL_CYN}"Made Odin recovery image: $(PRODUCT_OUT)/recovery.tar"${CL_RST}
